...
const PatientDashboard = () => {
  ...
  useEffect(() => {
    ...
    const fetchMedicines = async () => {
      const patientId = "patientIdPlaceholder";
      const res = await axios.get(`${API_URL}/api/medicines/patient/${patientId}`);
    ...
    <div className="container mx-auto px-4 py-8 mt-16">
        ...
        {appointments.length === 0 ? (
          ...
          <ul>
            {appointments.map((appt) => (
              <li key={appt.id}>{appt.date} - {appt.type} ({appt.status})</li>
            ))}
          </ul>
        )}
      </section>
      ...
      <section className="mb-8">
        ...
        {medicines.length === 0 ? (
          ...
          <ul>
            {medicines.map((med) => (
              <li key={med.id}>{med.name} ({med.dosage}) - {med.instructions}</li>
            ))}
          </ul>
        )}
      </section>
      ...
      <section className="mb-8">
        ...
        <div className="bg-gray-100 p-4 rounded-lg h-60 overflow-y-scroll mb-4">
          {messages.map((msg, idx) => (
            <div key={idx} className={`mb-2 ${msg.from === "patient" ? "text-right" : "text-left"}`}>
              <span className="inline-block px-3 py-1 rounded bg-blue-200">{msg.text}</span>
            </div>
          ))}
        </div>
        <form onSubmit={sendMessage} className="flex">
          ...
          />
          <button type="submit" className="bg-blue-600 text-white px-6 py-2 rounded-r hover:bg-blue-700">
            Send
          </button>
        </form>
      </section>
